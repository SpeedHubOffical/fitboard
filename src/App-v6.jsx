...
const PAGE_SIZE = 12;
const MAX_PHOTOS = 5;
const DONATION_URL = "https://paypal.me/fitsboard";
...
export default function App() {
  ...
  const [guestMode, setGuestMode] = useState(() => {
    try { return localStorage.getItem("fitboard-guest") === "1"; } catch { return false; }
  });
  ...
  async function checkBanned() {
    const { data } = await supabase.from("banned_users").select("user_id").eq("user_id", session.user.id).maybeSingle();
  ...
  function continueAsGuest() {
    setGuestMode(true);
    try { localStorage.setItem("fitboard-guest", "1"); } catch {}
  }

  function goToSignIn() {
    setShowAuthPrompt(false);
    setGuestMode(false);
    try { localStorage.removeItem("fitboard-guest"); } catch {}
  }
  ...
  async function toggleLiked(item) {
    if (!requireAuth("Sign in to like outfits")) return;
  ...
  async function toggleSaved(id) {
    if (!requireAuth("Sign in to save outfits")) return;
  ...
  function openReportOutfit(item) {
    if (!requireAuth("Sign in to report content")) return;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          setAnnouncementMessage(payload.new.message);
  ...
  function openEditProfile() {
    if (!requireAuth("Sign in to edit your profile")) return;
  ...
  if (session && banChecked && isBanned) {
    return (
      <div style={{ ...styles.page, background: t.bg, color: t.text, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: 30, textAlign: "center" }}>
        <div style={{ fontSize: 44, marginBottom: 12 }}>🚫</div>
        <div style={{ fontSize: 20, fontWeight: 900 }}>Your account has been banned</div>
        <div style={{ fontSize: 14, color: t.textMuted, marginTop: 12, lineHeight: 1.7, maxWidth: 320 }}>
          You can't use FitBoard while your account is banned. If you think this was a mistake, you can appeal by emailing{" "}
          <span style={{ color: t.accent, fontWeight: 700 }}>{ADMIN_EMAILS[0]}</span>.
        </div>
        <button
      ...
      <div style={{ ...styles.page, background: t.bg, color: t.text, display: "flex", flexDirection: "column", justifyContent: "center", padding: 24 }}>
        <style>{`
          * { box-sizing: border-box; }
          ...
          .auth-guest { background: none; color: ${t.textMuted}; border: 1px solid ${t.border}; }
        ...
        <button className="auth-btn auth-guest" onClick={continueAsGuest}>
          Continue as guest
        </button>
  ...
  return (
    <div style={{ ...styles.page, background: t.bg, color: t.text }}>
      ...
      {tab === "profile" ? (
        !session ? (
          <>
            ...
            <div style={{ ...styles.empty, padding: "60px 24px" }}>
              <div style={{ fontSize: 30, marginBottom: 10 }}>👤</div>
              Sign in to see your profile, your uploads, and who you follow.
              <div style={{ marginTop: 18 }}>
                <button className="upload-cta" style={{ marginTop: 0 }} onClick={goToSignIn}>Sign in</button>
              </div>
            </div>
          </>
        ) : (
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {session ? (
                ...
                <button
                  className="icon-btn"
                  onClick={goToSignIn}
                  style={{ width: "auto", padding: "0 14px", background: t.accent, border: "none", color: "#fff", fontWeight: 700, fontSize: 13 }}
                >
                  Sign in
                </button>
              )}
      ...
      {tab === "feed" && (
        <button className="fab" onClick={() => { if (!requireAuth("Sign in to post an outfit")) return; resetForm(); setShowModal(true); }} aria-label="Add outfit">
      ...
      <nav className="bottom-nav">
        ...
        <button className={`nav-btn ${tab === "profile" ? "active" : ""}`} onClick={() => { if (!requireAuth("Sign in to see your profile")) return; setTab("profile"); fetchUserOutfits(session.user.id).then(setOwnUploads); }}><User size={20} /> Profile</button>
      </nav>
          ...
          <div className="detail-card" onClick={(e) => e.stopPropagation()} style={{ maxHeight: "80vh" }}>
            <div style={{ padding: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "14px 0" }}>
                ...
                {(!session || viewingProfileId !== session.user.id) && (
                  <button
      ...
      {showSendAnnouncement && isAdmin && (
        <div className="modal-backdrop" onClick={() => setShowSendAnnouncement(false)}>
          <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
            <div className="sheet-header">
              <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800 }}>Send announcement</h2>
              <button onClick={() => setShowSendAnnouncement(false)} style={{ background: "none", border: "none", color: t.textMuted, cursor: "pointer" }}>
                <X size={22} />
              </button>
            </div>
            <div style={{ fontSize: 13, color: t.textMuted, marginTop: 10 }}>
              This pops up live on the screen of anyone currently using the app.
            </div>
            <div className="field-label">Message</div>
            <textarea
              className="field-input"
              style={{ minHeight: 100, resize: "vertical", fontFamily: "inherit" }}
              placeholder="e.g. New styles just dropped — check the feed!"
              value={announcementDraft}
              onChange={(e) => setAnnouncementDraft(e.target.value)}
            />
            <button className="upload-cta" onClick={sendAnnouncement} disabled={announcementSending || !announcementDraft.trim()}>
              {announcementSending ? "Sending…" : "Send to everyone"}
            </button>
          </div>
        </div>
      )}
      ...
      {showAnnouncement && (
        <div className="modal-backdrop" onClick={() => setShowAnnouncement(false)}>
          <div className="modal-sheet" onClick={(e) => e.stopPropagation()} style={{ textAlign: "center" }}>
            ...
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800 }}>Announcement</h2>
            <div style={{ fontSize: 14, color: t.text, marginTop: 10, lineHeight: 1.6 }}>{announcementMessage}</div>
            <button className="upload-cta" onClick={() => setShowAnnouncement(false)}>Got it</button>
          </div>
        </div>
      )}
      ...
      {showAuthPrompt && (
        <div className="modal-backdrop" onClick={() => setShowAuthPrompt(false)}>
          <div className="modal-sheet" onClick={(e) => e.stopPropagation()} style={{ textAlign: "center" }}>
            ...
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800 }}>{authPromptMessage || "Sign in to continue"}</h2>
            <div style={{ fontSize: 13.5, color: t.textMuted, marginTop: 8 }}>
              You're browsing as a guest. Sign in to unlock this.
            </div>
            <button className="upload-cta" onClick={goToSignIn}>Sign in</button>
            <div style={{ marginTop: 14, fontSize: 13, color: t.textFaint, cursor: "pointer" }} onClick={() => setShowAuthPrompt(false)}>
              Keep browsing
            </div>
          </div>
        </div>
        ...
        <div className="modal-backdrop" onClick={() => setShowSettings(false)}>
          <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 12, color: t.textFaint, marginTop: 10 }}>
              {session ? <>Signed in as {session.user.email}{isAdmin ? " · Admin" : ""}</> : "Browsing as guest"}
            </div>
            ...
            {isAdmin && (
              <div className="settings-row" onClick={() => { setShowSettings(false); setShowBannedPanel(true); loadBannedUsers(); }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <Ban size={17} color={t.accent} />
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 14 }}>Banned users</div>
                    <div style={{ fontSize: 12, color: t.textMuted }}>Manage bans, unban people</div>
                  </div>
                </div>
                <span style={{ color: t.textFaint }}>›</span>
              </div>
            ...
            {isAdmin && (
              <div className="settings-row" onClick={() => { setShowSettings(false); setShowSendAnnouncement(true); }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <Bell size={17} color={t.accent} />
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 14 }}>Send announcement</div>
                    <div style={{ fontSize: 12, color: t.textMuted }}>Pops up live on everyone's screen</div>
                  </div>
                </div>
                <span style={{ color: t.textFaint }}>›</span>
              </div>
            ...
            {session ? (
              ...
              <div className="settings-row" onClick={goToSignIn}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <User size={17} color={t.accent} />
                  <div style={{ fontWeight: 700, fontSize: 14, color: t.accent }}>Sign in</div>
                </div>
              </div>
            )}
          </div>
